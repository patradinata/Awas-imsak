...
const Quran = () => {
  ...
  // Setelah 3 detik, tampilkan teks
  useEffect(() => {
  ...
  return (
    <>
      <div className="flex flex-col justify-center items-center">
        {loading && <div className="w-12 h-12 border-4 border-t-4 border-[#3e664e] animate-spin"></div>}
        {showText && <div className="text-center mt-5">Sabar ya, kan lagi puasaaa....</div>}
        {!loading && !showText && (
          <>
            <div className="flex flex-col max-[640px]:w-[100%]">
              ...
              <div className="relative">
                <div className="flex items-center">
                  <input type="text" placeholder="Search Surah..." value={searchTerm} onChange={handleSearch} className="parent bg-[#0d1811] border border-[#3e664e] text-white px-[2.4rem] py-2 rounded-lg w-[20%] max-[640px]:w-[70%] mb-5" />
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <FontAwesomeIcon icon={faSearch} className="text-white -mt-5" /> {/* Tambahkan class -mt-1 */}
                  </div>
                </div>
              </div>

              {searchResults.length === 0 && (
                <div className="bg-[#0d1811] border border-[#3e664e] p-4 rounded-2xl w-[500px] flex flex-col justify-center items-center text-center">
                  ...
                  <Link key={index} href={"/tadarus/surah/" + surahData.number}>
                    <div className="bg-[#0d1811] border border-[#3e664e] px-4 py-3 rounded-lg cursor-pointer">
                      <div className="flex justify-between space-x-[5rem]">
                        <p className="text-white font-bold text-lg mb-2 max-[640px]:text-[20px]">{surahData.asma.id.short}</p>
