...
const QuranContent = ({ loadedSurahs, searchResults, currentSurahs, loading, loadingProgress, searchTerm, handleSearch, handlePageChange, pageCount, surahsPerPage, pageRangeDisplayed, marginPagesDisplayed }: any) => {
  return (
...
const Quran = () => {
  ...
  useEffect(() => {
    ...
    const results = loadedSurahs.filter((surahData: SurahData) => surahData.asma.id.short.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="flex flex-col justify-center items-center">
      <Suspense fallback={<LoadingSpinner />}>
        {loading && loadedSurahs.length === 0 ? (
          <div className="flex flex-col justify-center items-center">
            <div className="w-12 h-12 border-4 border-t-4 border-[#3e664e] animate-spin"></div>
            <div className="text-center mt-3">Loading... {loadingProgress}%</div>
          </div>
        ) : (
          <QuranContent
